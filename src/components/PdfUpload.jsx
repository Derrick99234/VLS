...
function PdfUpload({ setOpenModal }) {
  ...
  function uploadVideo() {
    if (!formData.title || !formData.pdf || !formData.description) {
    ...
    const pdfRef = ref(
      storage,
      `uploadedPdf/${Date.now()}${formData.pdf.name}`
    );
    const pdfUpload = uploadBytesResumable(pdfRef, formData.pdf);
    ...
    pdfUpload.on(
      ...
      async () => {
        try {
          ...
          const uploadRef = collection(db, "PDF");
          await addDoc(uploadRef, {
            title: formData.title || "No PDF title",
          ...
          // Show success toast
          toast("PDf uploaded successfully", { type: "success" });
        } catch (error) {
          console.error(
          ...
          alert("An error occurred while updating Pdf");
        }
      }
    );
  }
    ...
    <>
          <section className="fixed bg-black/50 top-0 left-0 bottom-0 right-0">
            ...
            <div className="w-[300px] h-[500px] bg-white rounded-md absolute top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] items-center p-3 overflow-hidden pt-7">
              ...
              <button
                onClick={uploadVideo}
                className="bg-purple-500 rounded-md p-2 w-full text-white font-bold mt-3"
              >
                submit
              </button>
            </div>
